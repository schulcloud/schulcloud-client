...
window.addEventListener("DOMContentLoaded", function () {
	...
	if (filterModule) {
		filterModule.addEventListener("newFilter", (e) => {
			...
			const newurl = `?q=${
				document.querySelector(".search-field").value
			}&filterQuery=${escape(JSON.stringify(filter[0]))}`;
			softNavigate(newurl, ".ajaxcontent", ".pagination");
		});
		document
	...
	if (document.querySelector("#filter") && searchField) {
		searchField
			.closest("form")
			.addEventListener("submit", loadSearchResult);
	}
});
...
$(document).ready(function () {
	...
	$(".external-link").on("click", function () {
		...
		$externalLinkModal
			.find(".external-link-btn")
			.attr("href", $(this).data("external-link"));
	...
	$modals
		.find(".close, .btn-close, .external-link-btn")
		.on("click", function () {
			$modals.modal("hide");
		});
});
